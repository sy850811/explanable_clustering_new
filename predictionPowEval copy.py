...
def unseen_predictions(distanceMetric): 
    ...
    ourOwnPredictions = ownPredictions(eval_df,distanceMetric) 
...
# def ownPredictions(annotated_unseen_documents,distanceMetric):
    """
    Given: annotated unseen documents
    Format: [[c1,c2,c3], #doc1
            [c1,c2,c3],  #doc2
            [c1,c2,c3]   #doc3
    To Find: predictions for each document
    Solution:
    1) get the concept embeddings for each concept
    2) get the similarity of each concept with each cluster center
    3) get the assignment score for each cluster

    Note: #Pro - the code handles if we get a new concept in the unseen document
          #Con - code does not encorporate neighbours of concepts
    """

    #We are provided with the extract the concepts from the annotated documents
    ...
    # #load the concept embeddings
    # with open(fileName, 'r') as file:
    #     concept_embeddings = json.load(file)
    # document_predictions = []
    # for doc in annotated_unseen_documents:
    #     doc_assignment_scores = [0 for i in range(len(centroids))]
    #     for concept in doc:
    #         concept_embedding = concept_embeddings[concept]
    #         for index,centroid in enumerate(centroids):
    #             if distanceMetric =="cosine":
    #                 doc_assignment_scores[index]+=1 - cosine_distance(concept_embedding,centroid)
    #             else:
    #                 doc_assignment_scores[index]+= 1/(1+ euclidean_distance(concept_embedding,centroid))
    #     document_predictions.append(doc_assignment_scores.index(max(doc_assignment_scores)))
...
def ownPredictions(eval_df,distanceMetric):
    ...
    for doc_no,document in enumerate(documents): 
        ...
        #find index of concepts in the document
        ...
        for concept in concepts:
            #calculate context embedding for each concept
            support_word_index_list = concept_support[concept]["wordsUsed"]
            concept_embedding = tf.reduce_mean(tf.gather(document_embedding, support_word_index_list),axis=0).numpy().astype(np.float64).reshape(-1,)
            #calculate assignmentscore based on similarity of each concept with each centroid
            for index,centroid in enumerate(centroids):
                if concept_embedding.shape != centroid.shape:
                    print("Shape of concept_embedding:", concept_embedding.shape)
                    print("Shape of centroid:", centroid.shape)
                    print("support_word_index_list",support_word_index_list)
                    print("tf.reduce_mean(tf.gather(document_embedding, support_word_index_list),axis=1)",tf.reduce_mean(tf.gather(document_embedding, support_word_index_list),axis=1))

                try:
                    if distanceMetric =="cosine":
                        document_assignment_scores[index]+=1 - cosine_distance(concept_embedding,centroid.reshape(-1,))
                    else:
                        document_assignment_scores[index]+= 1/(1+ euclidean_distance(concept_embedding,centroid.reshape(-1,)))
                except RuntimeWarning as e:
                    print(f"Warning occurred at concept {concept}: {e}")
                                    # print mean of concept embedding
                    print("mean",tf.reduce_mean(concept_embedding))
                    #print max value of concept embedding
                    print("max",tf.reduce_max(concept_embedding))
                    #print min value of concept embedding
                    print("min",tf.reduce_min(concept_embedding))

        document_predictions[doc_no] = document_assignment_scores.index(max(document_assignment_scores))
    return document_predictions
            #calculate assignment score for each centroid
    # _,centroids,_ = load.clusters_n_centroids_n_labels()
    # fileName = os.path.join(root_address +env.DATASET_NAME+ "/concept_embeddings_testing_data.json")
    # #load the concept embeddings
    # with open(fileName, 'r') as file:
